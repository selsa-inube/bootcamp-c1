...
//function defined earlier in the chapter
...
function dominantDirection(words) {
    let recount = countBy(words, c => {
        let scriptValue = characterScript(c.codePointAt(0));
        if (scriptValue) {
            return scriptValue.direction;
        }
        return "No found";
    })
    
    if (recount.length == 0) {
        return "write a phrase or word";
    }
    return recount.reduce((a, b) => a.count > b.count ? a : b).name;
